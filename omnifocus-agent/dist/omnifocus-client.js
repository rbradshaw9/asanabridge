...
exports.OmniFocusClient = void 0;
...
class OmniFocusClient {
    ...
    detectVersion() {
        try {
            ...
            (0, child_process_1.execSync)('osascript -e "tell application \\"OmniFocus 4\\" to get version"', {
                stdio: 'ignore'
            });
            return '4';
        }
            ...
            try {
                ...
                (0, child_process_1.execSync)('osascript -e "tell application \\"OmniFocus 3\\" to get version"', {
                    stdio: 'ignore'
                });
                return '3';
            }
    ...
    runAppleScript(script) {
        try {
            const result = (0, child_process_1.execSync)(`osascript -e "${script.replace(/"/g, '\\"')}"`, {
                encoding: 'utf8',
                timeout: 30000
            });
            return result.trim();
    ...
    async createTask(projectName, taskName, options = {}) {
        this.launch();
    ...
    async updateTask(taskId, updates) {
        ...
        let updateClauses = [];
        if (updates.name)
            updateClauses.push(`set name of targetTask to "${updates.name}"`);
        if (updates.note !== undefined)
            updateClauses.push(`set note of targetTask to "${updates.note}"`);
        if (updates.completed !== undefined)
            updateClauses.push(`set completed of targetTask to ${updates.completed}`);
    ...
    parseAppleScriptList(result, type) {
        // This is a simplified parser - in production you'd want more robust parsing
        // For now, return empty array as placeholder
        return [];
    }
    parseAppleScriptRecord(result, type) {
        // This is a simplified parser - in production you'd want more robust parsing
...
exports.OmniFocusClient = OmniFocusClient;
